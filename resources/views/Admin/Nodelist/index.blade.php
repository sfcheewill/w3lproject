...
@section('content')
  ...
  <div class="mws-panel grid_8"> 
   ...
   <div class="mws-panel-body no-padding"> 
    <div id="DataTables_Table_1_wrapper" class="dataTables_wrapper" role="grid">
     ...
     <form action="/adminnodelist" method="get">
     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 334px;">权限名</th>
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 334px;">控制器</th>
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 334px;">方法</th>
      ...
      @foreach($data as $value)
       <tr class="odd"> 
        ...
        <td class=" ">{{$value->mname}}</td> 
        <td class=" ">{{$value->aname}}</td> 
        <td class=" "><a class="cstatus" href="javascript:void(0)">{{$value->status}}</a></td> 
        <td class=" ">
        <form action="/adminnodelist/{{$value->id}}" method="post" style="display: inline-block;">
          {{method_field('DELETE')}}
          {{csrf_field()}}
          <button class="btn btn-danger del">删除</button>
        </form>
            <a href="/adminnodelist/{{$value->id}}/edit" class="btn btn-info">修改</a>
        </td> 
       </tr>
       @endforeach
      </tbody>
 ...
 <script>
      ...
      $('.cstatus').click(function(){
        ...
        $.getJSON('/nodeedit',{id:id,status:status},function(data){
            if(data.code == '1'){
              o.html(data.status);
            }
          });
